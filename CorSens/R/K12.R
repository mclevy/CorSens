...
#' @param sigma \code{NULL} (default) The marginal distributions' covariance matrix, specified when no data is given, and when \code{k} and \code{mu} are provided (such as in a simulation). A covariance matrix, instead of a correlation matrix, is required for arbitrary-MVN distribution mapping (see \code{L86} function).
...
K12 <- function(data=NULL, distributions=NULL, parameters = NULL, N = 10^4, sigma = NULL, model, k=NULL, mu = NULL, threshold = c(0.01,0.05,0.1), ...){
  ...
  sigma <- round(sigma,2) # round to two significant figures
